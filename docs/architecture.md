...
The original CLI remains the stable minimum slice. A dedicated Tkinter Issue adds an optional desktop GUI prototype over the same Human EAST / selected Policy x3 composition, using a viewer-relative table; MJX's observation visualizer informed that presentation approach, but no MJX code, font, artwork, proto, or state model is included. A later Issue replaced the GUI's text-based tile display with vendored FluffyStuff/riichi-mahjong-tiles (public domain / CC0 1.0) PNG images, resolved by canonical tile label through a small `lisjong_play.tile_images` registry; see `src/lisjong_play/assets/tiles/THIRD_PARTY_NOTICE.md` for the pinned source revision and license provenance, which is separate from this repository's own MIT license. Hand, drawn-tile, river, meld, and dora-indicator presentation semantics (concealed/drawn separation, legal-discard click-to-select, tsumogiri / riichi-declaration / called-discard river markers) are unchanged by the switch to images. A later Issue made the table compact: the shared renderer takes a `BoardTileImages` bundle of per-usage registries (hand, board-side melds / dora indicators, river, and desaturated tsumogiri river tiles), and seats are placed around the center of the felt with their text and melds on the outer side and their river facing the middle. Tsumogiri is shown by graying the tile's white face rather than by a text marker, so the GUI carries its own river legend while the CLI keeps its `*` text marker. None of this changes the six-tiles-per-row river layout, the canonical river model, or what the viewer is allowed to read from the engine.
