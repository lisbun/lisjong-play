...
The same assets are displayed at three sizes, largest where the player acts: hand and drawn tiles are the biggest because they are the click targets, river tiles come next because rivers are read constantly, and melds and dora indicators are the smallest, so long rivers stay on the table without pushing the hand area off screen. Tsumogiri discards are drawn from a fourth registry holding desaturated copies of the river-size images, which is what marks them in the river instead of a text marker. Each size has its own `TileImageRegistry`, so a tile label resolves to one cached image object per size and the Tk `PhotoImage` references stay alive for the application lifetime. Live Human Play and the Replay Viewer share this behavior through the same board renderer.
